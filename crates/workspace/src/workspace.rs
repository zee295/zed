...
impl Workspace {
    ...
    pub(crate) fn resize_dock_at(
        ...
        if self.previous_dock_drag_coordinates == Some(pointer_position) {
            return;
        }
        self.previous_dock_drag_coordinates = Some(pointer_position);

        let new_size = match dock_pos {
            DockPosition::Left => pointer_position.x - self.bounds.left(),
            DockPosition::Right => self.bounds.right() - pointer_position.x,
            DockPosition::Bottom => self.bounds.bottom() - pointer_position.y,
        };
        self.resize_dock(dock_pos, new_size, window, cx);
        self.serialize_workspace(window, cx);
    }

...
use gpui::{
    ...
    ManagedView, MouseButton, Pixels, Render, Subscription, Task, TaskExt, TouchDragEvent,
    TouchPhase, WeakEntity, Window, WindowId, actions, deferred, px,
};
            ...
            self.sidebar.as_ref().map(|sidebar_handle| {
                ...
                let resize_handle = deferred(
                    div()
                        ...
                        .on_touch_drag({
                            let weak = weak.clone();
                            move |event: &TouchDragEvent, window, cx| {
                                match event.phase {
                                        ...
                                        weak.update(cx, |this, cx| {
                                            if let Some(sidebar) = &this.sidebar {
                                                let new_width = if sidebar_on_right {
                                                    window.bounds().size.width - event.position.x
                                                } else {
                                                    event.position.x
                                                };
                                    ...
                                    TouchPhase::Ended => {
                                        weak.update(cx, |this, cx| this.serialize(cx)).ok();
                                    }

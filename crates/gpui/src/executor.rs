...
impl ForegroundExecutor {
    ...
    /// Poll an immediately-ready future once on WebAssembly.
    ///
    /// The browser main thread cannot block without preventing promise and event
    /// callbacks from making progress.
    ...
    /// Poll once on WebAssembly and return pending work to the asynchronous path.
    #[cfg(target_family = "wasm")]
    pub fn block_with_timeout<R, Fut: Future<Output = R>>(
        &self,
        _duration: Duration,
        future: Fut,
    ) -> Result<R, std::pin::Pin<Box<Fut>>> {

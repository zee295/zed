...
impl Uri {
    ...
    /// Create a URI from a file path.
    ///
    /// On wasm the path is the virtual absolute path used by RemoteFs
    /// (e.g. `/workspace/demo/src/main.rs` → `file:///workspace/demo/src/main.rs`).
    pub fn from_file_path<P: AsRef<std::path::Path>>(path: P) -> Result<Self, ()> {
        #[cfg(target_family = "wasm")]
        {
            // url_wasm implements from_file_path for wasm32; prefer that so
            // encoding/host handling stays consistent with Url.
    ...
    pub fn to_file_path(&self) -> Result<std::path::PathBuf, ()> {
        ...
        // path() drops the host and yields the absolute path (e.g. /workspace/...).
        Ok(std::path::PathBuf::from(self.0.path()))
    }
}
...
mod tests {
    use super::*;
    use crate::{Location, Position, Range, TextDocumentIdentifier, TextEdit, WorkspaceEdit};
    use serde_json;
    ...
    fn test_from_str_and_deserialize_consistency() {
        ...
        for uri_str in test_cases {
            ...
            let deserialized_uri: Uri =
                serde_json::from_str(&json_str).expect("Should deserialize");

            // They should be equal
            assert_eq!(
                from_str_uri, deserialized_uri,
                "from_str and deserialize should produce the same Uri for: {}",
                uri_str
            );
            ...
            assert_eq!(
                from_str_uri.as_str(),
                deserialized_uri.as_str(),
                "Both should have the same underlying URL string for: {}",
                uri_str
            );
        }
    }
    ...
    fn test_uri_with_brackets_serialization() {
        ...
        let location_serialized =
            serde_json::to_string(&location).expect("Should serialize Location");
    ...
    fn test_from_file_path_consistency_with_from_str() {
        ...
        for path_str in test_paths {
            ...
            // They should be equal
            assert_eq!(
                uri_from_path, uri_from_str,
                "URI from file path and from string should be equal for: {}",
                path_str
            );

            // Both should serialize the same way (with encoded brackets if present)
            let serialized_from_path =
                serde_json::to_string(&uri_from_path).expect("Should serialize");
            let serialized_from_str =
                serde_json::to_string(&uri_from_str).expect("Should serialize");
            assert_eq!(
                serialized_from_path, serialized_from_str,
                "Serialization should be identical for: {}",
                path_str
            );
        }
    }

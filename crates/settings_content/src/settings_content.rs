...
mod tests {
    ...
    #[test]
    fn default_user_settings_are_serializable() {
        let (settings, status) = UserSettingsContent::parse_json(
            r#"{
                "project_panel": { "dock": "left", "auto_fold_dirs": false }
            }"#,
        );
        assert_eq!(status, ParseStatus::Success);
        let mut settings = settings.expect("empty settings object should parse");
        settings.content.agent.get_or_insert_default().dock = Some(DockPosition::Right);
        settings.content.project_panel.get_or_insert_default().dock = Some(DockSide::Left);
        settings.content.outline_panel.get_or_insert_default().dock = Some(DockSide::Left);
        settings
            .content
            .collaboration_panel
            .get_or_insert_default()
            .dock = Some(DockPosition::Left);
        settings.content.git_panel.get_or_insert_default().dock = Some(DockPosition::Left);

        serde_json::to_value(settings).expect("default user settings should serialize to JSON");
    }
}
